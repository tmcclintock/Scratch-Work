...
for z in zs:
    ...
    MF = TMF.tinker_mass_function(cosmo_dict, redshift=z)
    MF.set_parameters(1.97,1.0,0.51,1.228)
    params = np.array([1.97,1.0,0.51,1.228])
    lM = np.log(me)
    dndlM = np.array([MF.dndlM(lMi) for lMi in lM])
    plt.plot(me, dndlM/emf,label="Eduardo")
    plt.plot(me, dndlM/t08,label="T08")
    plt.plot(me, dndlM/t10,label="T10")
    print np.mean((dndlM-t10)/t10)
    plt.xscale('log')
    plt.ylabel("dn/dlnM")
    plt.xlabel("Mass")
