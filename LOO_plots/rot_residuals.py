"""
Show the residuals of all rotated emulators, or just the bottom panels.
"""
import numpy as np
...
make_LOOrot = True
LOOrot_path = "saved_files/%s_LOOrot_curves.p"%name
LOOrot = []
if make_LOOrot:
    for i in range(0, N_cosmos):
        ...
        for j in range(N_z):
            ...
            LOOrot_cos.append(N_emu)
        LOOrot.append(LOOrot_cos)
    pickle.dump(LOOrot, open(LOOrot_path, "wb"))
LOOrot = pickle.load(open(LOOrot_path, "rb"))
...
fig.savefig("fig_LOOrot_residuals.pdf")
plt.show()
